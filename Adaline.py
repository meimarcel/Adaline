...
class Adaline:
    ...
    def __init__(self, path, number_features, col1=0, col2=-1, learning_rate=0.01, number_epoch=2000, bias=1, precision=0.000001):
    ...
    # 4 - Funcao para medir o erro quadratico medio (EQM)
    def __mse(self):
        mse = 0
        for i in range(len(self.data_training)):
            result = self.__w.dot(self.data_training[i])
            mse += ((self.data_results[i] - result) ** 2)
        return mse / len(self.data_training)

    # 5 - Funçao para começar o treino
    def start_train(self):
        # 5.1 - Inserir o bias no dataset dos valores e no dos pesos
        ...
        # 5.2 - Rodar o treinamento dentro do limite das epocas inseridas no construtor
        for i in range(self.number_epoch):
            print("Interaçao: {}".format(i))
            previous_mse = self.__mse()

            # 5.3 - Percorrer todas as linhas do dataset
            for j in range(len(self.data_training)):
                # 5.4 - Fazer o produto escalar dos valores da linha atual do dataset com o vetor que armazena os pesos
                result = self.__w.dot(self.data_training[j])

                # 5.5 - Realizar os ajuste nos pesos
                error = self.data_results[j] - result
                self.__w += (self.learning_rate * error * self.data_training[j])

            current_mse = self.__mse()
            self.errors.append(current_mse)
            # 5.6 - Se o EQM atual subtraido pelo EQM anterior for menor que a precisao passada, o treinamento e terminado
            if abs(current_mse - previous_mse) <= self.precision:
                print("Fim das interacoes: {}".format(i))
                break

    # 6 - Funçao para prever a qual a classe de determinados valores
    def predict(self,values):  # Pode receber um vetor, ou uma matriz
        # 6.1 - Verificar a dimençao da matriz
        if numpy.ndim(values) == 1:
            # 6.2 - Inserir o bias
            values = numpy.insert(values, len(values), 1)
            # 6.3 - Fazer o produto escalar e verificar a saida desse valor
    ...
    # 7 - Funcao para plotar um grafico contendo os dois primeiros valores do dataset em plano bidimensional
    def plot(self):
        ...
        # 7.1 - Verificar o numero de pesos para saber se o treino foi realizado em um dataset bidimensional
        # Se sim, plotar a reta que separa as classes
        if (len(self.__w) == 3):
            if self.__w[0] < 0 and self.__w[1] < 0:
                x = [(((min(self.data_training[0:, 1]) * self.__w[1]) + self.__w[-1]) / -self.__w[0]),
                     min(self.data_training[0:, 0])]
                y = [min(self.data_training[0:, 1]),
                     (((min(self.data_training[0:, 0]) * self.__w[0]) + self.__w[-1]) / -self.__w[1])]
                plt.plot(x, y, color='red')
            elif self.__w[0] < 0 and self.__w[1] > 0:
                x = [(((min(self.data_training[0:, 1]) * self.__w[1]) + self.__w[-1]) / -self.__w[0]),
                     max(self.data_training[0:, 0])]
                y = [min(self.data_training[0:, 1]),
                     (((max(self.data_training[0:, 0]) * self.__w[0]) + self.__w[-1]) / -self.__w[0:][1])]
                plt.plot(x, y, color='red')
            elif self.__w[0] > 0 and self.__w[1] > 0:
                x = [(((max(self.data_training[0:, 1]) * self.__w[1]) + self.__w[-1]) / -self.__w[0]),
                     max(self.data_training[0:, 0])]
                y = [max(self.data_training[0:, 1]),
                     (((max(self.data_training[0:, 0]) * self.__w[0]) + self.__w[-1]) / -self.__w[1])]
                plt.plot(x, y, color='red')
            elif self.__w[0] > 0 and self.__w[1] < 0:
                x = [(((max(self.data_training[0:, 1]) * self.__w[1]) + self.__w[-1]) / -self.__w[0]),
                     min(self.data_training[0:, 0])]
    ...
    # 8 - Funcao para plotar o comportamento do EQM
    def plot_errors(self):
        plt.plot([0, 0], [0, self.errors[0]], color="black")
        plt.plot([0, len(self.errors)], [0, 0], color="black")
        plt.plot(self.errors)
        print(self.errors)
        plt.show()

    # 9 - Funcao retornar a porcentagem de acertos de previsoes
    def show_accuracy(self):
        correct = 0
        # 9.1 - Fazer a previsao do dataset
        predictions = self.predict(self.data_training[0:, :-1])
        # 9.2 - Comparar os resultados da previsao com as saidas esperadas
        for x in range(len(self.data_results)):
            if self.data_results[x] == predictions[x]:
                correct += 1
        return float(correct) / len(self.data_results)

    # 10 - Funcao para retornar os pesos
    def getWeights(self):
        return self.__w
